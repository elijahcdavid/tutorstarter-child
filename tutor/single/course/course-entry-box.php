 ...
 * @since 1.0.0
 * 
 * CUSTOMIZED VERSION - Copy of plugin template for customization
 * This file overrides the plugin template from: plugins/tutor/templates/single/course/course-entry-box.php
 */
...
<div class="tutor-card tutor-card-md tutor-sidebar-card">
	...
	<div class="tutor-card-body">
		...
		if ( $is_enrolled || $is_privileged_user ) {
			...
			echo apply_filters( 'tutor_course/single/entry-box/is_enrolled', ob_get_clean(), get_the_ID() );//phpcs:ignore
		} elseif ( $is_public ) {
			...
			<?php
			echo apply_filters( 'tutor_course/single/entry-box/is_public', ob_get_clean(), get_the_ID() );//phpcs:ignore
		} else {
			...
			if ( tutor_utils()->is_course_fully_booked( null ) ) {
				...
				<?php
				echo apply_filters( 'tutor_course/single/entry-box/fully_booked', ob_get_clean(), get_the_ID() );//phpcs:ignore
				...
				echo apply_filters( 'tutor_course/single/entry-box/purchasable', ob_get_clean(), get_the_ID() );//phpcs:ignore
			} else {
				ob_start();
				?>
				...
				<?php
				echo apply_filters( 'tutor_course/single/entry-box/free', ob_get_clean(), get_the_ID() );//phpcs:ignore
			}
		}
